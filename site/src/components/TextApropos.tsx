

export const TextApropos = () => {
  return (
    <>
    <div className='ml-15 mt-40 justify-content-center'>

    <h1 className=' text-center text-justify...'>Présentation du groupe </h1>

    <p className='sm:flex ml-20  mt-20  mt-10 w-3/4 text-justify'> Lorem, ipsum dolor sit amet consectetur adipisicing elit. Quia tempora tempore autem hic, fugiat quae nemo qui rem unde quis
      voluptates eaque soluta quasi aliquam dolores cupiditate ea accusantium sequi!Lorem, ipsum dolor sit amet consectetur adipisicing elit. Quia tempora tempore autem hic, fugiat quae nemo qui rem unde quis
      voluptates eaque soluta quasi aliquam dolores cupiditate ea accusantium sequi</p>
  </div>


  <div className='mt-20'>

    <h2 className='text-center'> 1 Nos missions </h2>

    
  </div>


  <div className="sm:flex ml-20 mr-20 mt-20 justify-content-center">
    <div className="md:w-1/4 pr-8 ">
      <img src="https://via.placeholder.com/150" alt="Placeholde" className="sm:w-full mb-2 center"></img>
    </div>
    <div className="md:w-2/4 ">
      <p className="leading-7 text-justify">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nullam ut interdum sapien. Nulla commodo risus ut lacus maximus bibendum. Fusce commodo, nisl quis tristique faucibus, est velit ullamcorper nisl, quis finibus quam justo a magna. Sed efficitur sagittis nisi, eu tristique justo laoreet at. Nulla vel laoreet nulla. Donec suscipit nulla ut risus malesuada, ac tempor metus vulputate. Suspendisse at lacus sed velit malesuada dignissim a eget ex. Sed dignissim justo eget sapien pharetra tristique.</p>
    </div>
  </div>
   
   
  <div className='mt-20'>

    <h2 className='text-center mt-10 text-justify...'> 2 Notre histoire </h2>
  </div>


  <div className="sm:flex ml-20 mr-20 mt-20 justify-content-center">
    <div className="md:w-2/4 ">
      <p className="leading-7 text-justify">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nullam ut interdum sapien. Nulla commodo risus ut lacus maximus bibendum. Fusce commodo, nisl quis tristique faucibus, est velit ullamcorper nisl, quis finibus quam justo a magna. Sed efficitur sagittis nisi, eu tristique justo laoreet at. Nulla vel laoreet nulla. Donec suscipit nulla ut risus malesuada, ac tempor metus vulputate. Suspendisse at lacus sed velit malesuada dignissim a eget ex. Sed dignissim justo eget sapien pharetra tristique.</p>
    </div>
    <div className="md:w-1/4 pl-8">
      <img src="https://via.placeholder.com/150 " alt="Placeholde" className="sm:w-full mb-2"></img>
    </div>
  </div>


  <div className='mt-20'>

    <h2 className='text-center'> 3 Nos projets </h2>

  </div>
  <div className="sm:flex ml-20 mr-10 mt-20 justify-content-center">
    <div className="md:w-1/4 pr-8">
      <img src="https://via.placeholder.com/150" alt="Placeholde" className="sm:w-full mb-2"></img>
    </div>
    <div className="md:w-2/4">
      <p className="leading-7 text-justify">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nullam ut interdum sapien. Nulla commodo risus ut lacus maximus bibendum. Fusce commodo, nisl quis tristique faucibus, est velit ullamcorper nisl, quis finibus quam justo a magna. Sed efficitur sagittis nisi, eu tristique justo laoreet at. Nulla vel laoreet nulla. Donec suscipit nulla ut risus malesuada, ac tempor metus vulputate. Suspendisse at lacus sed velit malesuada dignissim a eget ex. Sed dignissim justo eget sapien pharetra tristique.</p>
    </div>
  </div>
    </>
  
  )
}
